...
export function CostStructureInsight({ onBack, userId }: CostStructureInsightProps) {
  ...
  const renderTable = () => {
    ...
    return (
      <>
      {/* Mobile: stacked cards grouped by section */}
      <div className="md:hidden space-y-4">
        {[
          { title: "Fixed (Time-Independent)", items: fixedItems, color: BREAKDOWN_COLORS.fixed, pct: fixedPercent },
          { title: "Variable (Usage-Dependent)", items: variableItems, color: BREAKDOWN_COLORS.variable, pct: variablePercent },
          { title: "Deferred (Future Wear)", items: deferredItems, color: BREAKDOWN_COLORS.deferred, pct: deferredPercent },
        ].map((section) => {
            ...
            <div key={section.title} className="space-y-2">
              <div className="rounded-md bg-muted/30 px-3 py-2 flex items-center justify-between">
                ...
                <div className="text-right whitespace-nowrap">
                  ...
                  <div className="text-xs text-muted-foreground">{section.pct.toFixed(1)}%</div>
                </div>
              </div>
                ...
                return (
                  <div key={`${item.name}-${idx}`} className="rounded-lg border bg-card p-3 ml-2">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="font-semibold whitespace-nowrap">{formatCurrency(item.amount, currency)}</div>
                    </div>
                  </div>
                );
              })}
