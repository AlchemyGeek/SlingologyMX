  ...
  const renderTable = () => {
    ...
    return (
      ...
      <div className="md:hidden space-y-4">
        {[
          { title: "Variable (Actual Transactions)", items: variableItems, color: CATEGORY_COLORS.variable },
          { title: "Fixed (Time-Based Amortized)", items: fixedItems, color: CATEGORY_COLORS.fixed },
          { title: "Deferred (Usage-Based Reserves)", items: deferredItems, color: CATEGORY_COLORS.deferred },
        ].map((section) => {
          if (section.items.length === 0) return null;
          const sectionTotal = section.items.reduce((s, i) => s + i.amount, 0);
          const sectionPerHr = hoursData && hoursData.hours > 0
            ? `${formatCurrency(sectionTotal / hoursData.hours, currency)}/hr`
            : "—";
          return (
            <div key={section.title} className="space-y-2">
              <div className="rounded-md bg-muted/30 px-3 py-2 flex items-center justify-between">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="inline-block w-3 h-3 rounded shrink-0" style={{ backgroundColor: section.color }} />
                  <span className="font-semibold text-sm truncate">{section.title}</span>
                </div>
                <div className="text-right whitespace-nowrap">
                  <div className="font-semibold text-sm">{formatCurrency(sectionTotal, currency)}</div>
                  <div className="text-xs text-muted-foreground">{sectionPerHr}</div>
                </div>
              </div>
              {section.items.map((item, idx) => {
                const itemPct = totalCost > 0 ? ((item.amount / totalCost) * 100).toFixed(1) : "0";
                const itemPerHr = hoursData && hoursData.hours > 0
                  ? `${formatCurrency(item.amount / hoursData.hours, currency)}/hr`
                  : "—";
                return (
                  <div key={`${item.name}-${idx}`} className="rounded-lg border bg-card p-3 ml-2">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 flex items-center gap-2 text-xs">
                          ...
                          <span className="text-muted-foreground">{itemPct}%</span>
                        </div>
                      </div>
                      <div className="text-right whitespace-nowrap">
                        <div className="font-semibold">{formatCurrency(item.amount, currency)}</div>
                        <div className="text-xs text-muted-foreground">{itemPerHr}</div>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="rounded-lg border-2 bg-muted/30 p-3 flex items-center justify-between">
          <div>
            <div className="font-semibold">Total (All-In)</div>
            <div className="text-xs text-muted-foreground mt-0.5">
              {trueCostPerHour !== null ? `${formatCurrency(trueCostPerHour, currency)}/hr` : "—"}
            </div>
          </div>
